...
# Create a file named "scripts/parser_location.txt" at the "en" directory and
...
for parser in `cat scripts/parser_location.txt`; do
  ...
  if [ ! -e ${parsed_dir}/${sentences_basename}.${parser_name}.jigg.xml ]; then
    ...
    # python -m depccg.tools.tagger --format jigg_xml ${parsed_dir}/${sentences_basename}.${parser_name}.tsgn.mod.ptb \
