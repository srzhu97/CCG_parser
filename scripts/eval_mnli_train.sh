...
results_dir="mnli_train_results"
...
function proving() {
  id=$1
  label=$2
  if [ -f ${plain_dir}/mnli_train_${id}_${label}.txt ]; then
    ./scripts/rte_train.sh ${plain_dir}/mnli_train_${id}_${label}.txt $templates $nbest
  fi
}
...
function display() {
  id=$1
  label=$2
  if [ -f ${plain_dir}/mnli_train_${id}_${label}.answer ]; then
    gold_filename="${plain_dir}/mnli_train_${id}_${label}.answer"
...
function calculate_score() {
  id=$1
  label=$2
  if [ -f ${cache_dir}/mnli_train_${id}_${label}.txt.tok ]; then
    count=$((count += 1))
    f="${cache_dir}/mnli_train_${id}_${label}.txt.tok"
    filename=${f##*/}
