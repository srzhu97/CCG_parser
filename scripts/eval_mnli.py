...
def prove_vampire(premises, conclusion, predicates, lst, axioms, mode):
    ...
    # TODO: MODIFIED
#     arg_lf = ARGS.sem.replace('cache/', '')
    arg_lf = ARGS.sem.replace('cache_dev/', '')
    arg_lf = arg_lf.replace('.sem.xml', '')
#     print(f"==vampire== premises {premises} conclusion {conclusion} predicates {predicates}") 
    with open("LF_dev/" + arg_lf + ".lf", "w", encoding="utf-8") as z:
...
#     TODO:
    arg = ARGS.sem.replace('cache_dev/', '')
