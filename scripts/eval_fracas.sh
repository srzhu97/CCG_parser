...
#
# Usage:
#  ./scripts/eval_fracas2.sh <nbest> <ncore> <templates> <list of section numbers>
#
# Example:
# ./scripts/eval_fracas2.sh 1 3 semantic_templates_en_comparatives.yaml 5 6
#
...
accuracy=$(cat ${results_dir}/score_section${sec}.txt | head -n 15 | awk -F'|' '$0 ~ "total"{print $2}' | sed 's/[ ]*//g')
...
function display() {
  ...
  for gold_filename in ${plain_dir}/${section}.answer; do
    ...
    for parser in "" "candc." "easyccg." "depccg."; do
      ...
      echo '<td><a style="background-color:'$color';" href="'${base_filename/.answer/.txt}.${parser}html'">'$system_answer'</a>' >> ${results_dir}/main_section${sec}.html
    done
    echo '</tr>' >> ${results_dir}/main_section${sec}.html
  done
}
...
" >> ${results_dir}/main_section"${sec}".html

score="${results_dir}/score_section${sec}.txt"
