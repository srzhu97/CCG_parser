...
plain_dir="plain_multinli_dev"
cache_dir="cache_dev"
# results_dir="en_results"
results_dir="mnli_dev_results"

if [ ! -e $results_dir ]; then
  mkdir -p $results_dir
fi

# TODO: mnli_dev, mnli_train
function proving() {
  id=$1
  label=$2
  if [ -f ${plain_dir}/mnli_dev_${id}_${label}.txt ]; then
    ./scripts/rte.sh ${plain_dir}/mnli_dev_${id}_${label}.txt $templates $nbest
  fi
}
...
function display() {
  id=$1
  label=$2
  if [ -f ${plain_dir}/mnli_dev_${id}_${label}.answer ]; then
    gold_filename="${plain_dir}/mnli_dev_${id}_${label}.answer"
    base_filename=${gold_filename##*/}
    gold_answer=`cat $gold_filename`
    echo '<tr>
    <td>'${base_filename/.answer/}'</td>
    <td>'$gold_answer'</td>' >> $results_dir/main_mnli.html
    for parser in "" "candc." "depccg."; do
      ...
      echo '<td><a style="background-color:'$color';" href="'${base_filename/.answer/.txt}.${parser}html'">'$system_answer'</a>' >> $results_dir/main_mnli.html
    done
    echo '</tr>' >> $results_dir/main_mnli.html
  fi
}
...
function calculate_score() {
  id=$1
  label=$2
  if [ -f ${cache_dir}/mnli_dev_${id}_${label}.txt.tok ]; then
    count=$((count += 1))
    f="${cache_dir}/mnli_dev_${id}_${label}.txt.tok"
    filename=${f##*/}
...
# TODO execute python script to run rte.sh
